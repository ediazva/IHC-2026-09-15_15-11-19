...
**`XR_ERROR_RUNTIME_UNAVAILABLE` / `xrCreateInstance failed` / "OpenXR cannot
start".**
No hay **runtime OpenXR** activo. Soluciones: activa
`Meta → Meta XR Simulator → Activate`, o abre Meta Quest Link, o bien cambia a
`XR Simulation` en `Project Settings → XR Plug-in Management`.

**No debo mezclar simuladores.**
El **XR Interaction Simulator** (XRI) y el **Meta XR Simulator** no pueden estar
activos a la vez. Quita uno: `Bomba VR → Quitar simulador XRI…`.

**Aviso: "A collider used by an Interactable object is already registered with
another Interactable object".**
`XRGrabInteractable` rellena `colliders` con **todos** los colliders hijos si no
se asignan. Se limita al cuerpo con
`Bomba VR → Reparar colliders de la bomba (quitar avisos)` o
ajustando `grab.colliders` a mano.

**Aviso: "The character \u2717 was not found in the [LiberationSans SDF]…".**
El símbolo `✓`/`✗` no está en la fuente TMP por defecto; se sustituye por `□`.
Es **inofensivo**. Para quitarlo: añade el glifo a la **font asset** o cambia el
texto (p. ej. "OK" / "MAL").

**La partida "se sale sola" / la bomba explota.**
No se cierra el juego: **explota por 3 strikes**. Filtra la consola por
`[Bomba VR] STRIKE` para ver **qué módulo** lo provoca.

**No me aparece el botón START del Simón.**
La escena guardada puede tener un panel antiguo. Ejecuta
`Bomba VR → Añadir módulo Simón a la bomba actual` (reconstruye con START) y
guarda.

**Mensajes varios de Meta (`ISDKEngineTelemetry native library not found`,
`MetaXROperator disabled after Play Mode`, etc.).**
Son avisos **informativos** de los SDKs, no errores del juego.

**El ratón no responde en Play.**
Con el **XR Interaction Simulator** activo, XRI desactiva el ratón/táctil para
la UI a propósito (usa la UI del simulador).

---

## Git y ramas

- Rama principal del prototipo: `comienzo-bomba-vr`.
- Trabajo del módulo Simón + botón START: `feature/simon-start`.

```bash
git checkout feature/simon-start
```

> **Seguridad**: no guardes *tokens* de GitHub en la URL del remoto ni en el
> historial. Usa `gh auth login`, SSH, o el *credential helper*
